...
const ProfileModal = ({ isOpen, onClose }) => {
  ...
  // 2. State untuk Foto Profil (Default: Avatar Kartun)
  const [photo, setPhoto] = useState('https://api.dicebear.com/7.x/avataaars/svg?seed=Yonaldi');
  
  // 3. Ref untuk mengakses input file rahasia
  const fileInputRef = useRef(null);

  if (!isOpen) return null;

  // 4. Fungsi Mengganti Foto (Preview)
  const handleImageChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      // Membaca file gambar dan mengubahnya jadi URL sementara
      const reader = new FileReader();
      reader.onloadend = () => {
        setPhoto(reader.result); // Update foto di layar
      };
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="bg-white dark:bg-[#1E1E1E] w-full max-w-md rounded-3xl shadow-2xl relative overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        {/* AREA FOTO PROFIL */}
        <div className="relative px-6 -mt-16 mb-6 flex justify-center">
          <div 
            className="relative group cursor-pointer"
            onClick={() => fileInputRef.current.click()} // 5. Klik Foto -> Buka File Explorer
          >
            {/* Bingkai Foto */}
        ...
        <div className="px-8 pb-8 space-y-5">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-3 bg-gray-50 dark:bg-gray-800 p-3 rounded-xl border border-transparent focus-within:border-primary focus-within:bg-white dark:focus-within:bg-gray-900 transition-all">
          ...
          <div className="pt-4 flex gap-3">
             <button 
                onClick={onClose}
                className="flex-1 bg-primary hover:bg-primary-dark text-white font-bold py-3 rounded-xl shadow-lg shadow-primary/30 active:scale-95 transition-all flex items-center justify-center gap-2"
              >
              <Save size={18} /> Simpan
            </button>
            <button className="px-4 py-3 bg-red-50 dark:bg-red-900/20 text-red-500 hover:bg-red-100 dark:hover:bg-red-900/40 rounded-xl font-bold transition-colors" title="Keluar">
              <LogOut size={20} />
            </button>
          </div>
